...
function addToBlogsPage(topic, date) {
  ...
  if (content.includes(topic.slug + '.html')) { console.log('ℹ️  Card already in blogs.html.'); return; }
...
function updateSitemap(topic, date) {
  ...
  if (sm.includes(url)) { console.log('ℹ️  URL already in sitemap.'); return; }
...
async function main() {
  ...
  // Force regenerate check
  const force = process.env.FORCE_REGENERATE === 'true';
  if (fs.existsSync(outputPath) && !force) {
    console.log(`ℹ️  ${topic.slug}.html already exists. Use FORCE_REGENERATE=true to override.`);
    process.exit(0);
  }
